...
public class RSyntaxTextAreaCodeAssist extends  RSyntaxTextArea{
	...
	public void addExternalMemberClickedListener(ExternalMemberClickedListener externalMemberClickedListener) {
		javaLinkGenerator.addExternalMemberClickedListener(externalMemberClickedListener);
	}
		...
		javaLinkGenerator = (JavaLinkGenerator) textArea.getLinkGenerator();
		{
			textArea .addKeyListener(new KeyAdapter() {
				public void keyReleased(KeyEvent e) {
					// log.info(""+e);
					if (e.getKeyCode() == createVar.getKeyCode()&&e.getModifiers()==0) {
						createDecl();
					}
					if (e.getKeyCode() == gotoDeclaration.getKeyCode()&&e.getModifiers()==0) {
						gotToDeclaration();
					}
				}
			});
		...
		try {
			...
			if (fullName.contains("$")) {
				...
				if (fullName.contains(".")) {
					...
					if(groovyLanguageSupport==null) {
						throw new NullPointerException("groovyLanguageSupport is null");
					}
					if(groovyLanguageSupport.getJarManager()==null) {
						throw new NullPointerException("JarManager is null");
					}
